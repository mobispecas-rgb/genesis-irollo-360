...
function setupMCP(app) {
  ...
  app.get("/mcp/sse", async (req, res) => {
    try {
      ...
      server.tool("indexador_forcar", "Forcar ciclo agora", {}, async () => {
        const r = await fetch(GENESIS_URL + "/api/indexador/forcar", { method: "POST" });
        const d = await r.json();
        return { content: [{ type: "text", text: JSON.stringify(d, null, 2) }] };
      });
      server.tool("indexador_parar", "Parar o indexador", {}, async () => {
        const r = await fetch(GENESIS_URL + "/api/indexador/parar", { method: "POST" });
        const d = await r.json();
        return { content: [{ type: "text", text: JSON.stringify(d, null, 2) }] };
      });
      server.tool("indexador_iniciar", "Reiniciar o indexador", {}, async () => {
  ...
  app.use("/mcp/messages", async (req, res) => {
